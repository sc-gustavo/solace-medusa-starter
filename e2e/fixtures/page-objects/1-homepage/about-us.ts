...
async checkOurStorySection() {
    ...
    const ourStoryHeading = this.page.getByRole('heading', { name: 'Our story' })
...
async checkWhyUsSection() {
    ...
    const whyUsBoxes = this.page.locator('h3').filter({ hasText: /Timeless design|Exceptional quality|Sustainable practices|Tailored solutions/ });
...
async checkOurCraftsmanshipSection() {
    ...
    const ourCraftsmanshipHeading = this.page.getByRole('heading', { name: 'Our craftsmanship' })
...
async checkGetInspiredSection() {
    ...
    const getInspiredHeading = this.page.getByRole('heading', { name: 'Get inspired' })
    ...
    // check section boxes
    const getInspiredBoxes = this.page.locator('.flex.min-w-40').filter({ hasText: /Maximizing small spaces|Caring for wood furniture|Trends in designer furniture/});
