...
async checkHeaderAndFormComponent() {
    ...
    //check header
    const signUpPageHeader = await this.page.getByRole('heading', { name: 'Create account' })
...
async checkInvalidPasswordCharacters() {
    ...
    const atLeast8CharactersError = await this.page.locator('p').filter({ hasText: 'At least 8 characters' })
...
async checkPasswordLackOfUpperCase() {
    ...
    const upperCaseError = await this.page.locator('p').filter({ hasText: 'One uppercase letter' })
...
async checkPasswordLackOfNumberOrSymbol() {
    ...
    const numberOrSymbolError = await this.page.locator('p').filter({ hasText: 'One number or symbol' })
